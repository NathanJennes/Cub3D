...
/*   Updated: 2022/05/20 13:35:15 by njennes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ui_component	*uic_below(t_ui_component *comp, t_ui_component *other)
{
	...
	comp->pos.y = other->pos.y + other->size.y
		+ ft_maxi(comp->pad_up, other->pad_down);
	return (comp);
}
...
{
	comp->pos.x = other->pos.x + other->size.x / 2 - comp->size.x / 2;
	comp->pos.y = other->pos.y - comp->size.y
		- ft_maxi(comp->pad_down, other->pad_up);
	return (comp);
}

t_ui_component	*uic_side_left(t_ui_component *comp, t_ui_component *other)
{
	comp->pos.x = other->pos.x - comp->size.x
		- ft_maxi(comp->pad_right, other->pad_left);
...
{
	comp->pos.x = other->pos.x + other->size.x
		+ ft_maxi(comp->pad_left, other->pad_right);
	comp->pos.y = other->pos.y + other->size.y / 2 - comp->size.y / 2;
	return (comp);
}
