...
/*   Updated: 2022/06/16 19:27:20 by njennes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	parser->error_message = error;
	parser->column = ft_strlen(parser->line) - ft_strlen( infos->map_raw[pos.y] + pos.x);
...
int	map_print_error_layout(t_map_parser *parser)
{
	...
	while (it.y < parser->infos->height)
	{
		...
		if (it.y + parser->map_line_offset == parser->error_line_number)
		{
			it.x = 0;
